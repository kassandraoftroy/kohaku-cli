...
> [!IMPORTANT]
> NOTICE: this CLI has **NOT BEEN AUDITED** and **IS UNDER ACTIVE DEVELOPMENT** and **HAS BEEN WRITTEN WITH HELP FROM CLAUDE** et al. **DO NOT USE ON MAINNET** unless you know **EXACTLY WHAT YOU ARE DOING** and understand **YOU RISK CATASTROPHIC LOSS OF FUNDS**.
...
See [Kohaku CLI Wiki](https://kohaku.gwei.domains) for a practical guide to using kohaku-cli as a wallet solution "in real life"
...
**Stealth recipients:** `--to s0` (or another stored stealth selector) works for Tornado only as a single-note unshield with no `--tail-calls` (direct to the stealth address). Tornado `--tail-calls` and multi-note amounts need `--next` or a stored public HD `--to`. Railgun ERC-20 without `--tail-calls` can also go to a custom (non-wallet) address. Railgun native ETH unshield and `--tail-calls` still need a recipient whose key is in this wallet (`--next`, stored public/stealth address, or `sN`), because unwrap / tails run on that EIP-7702 account.
